...
def executar_experimento_hiperparametro(
    ...
    ----------
    nome_hiperparametro : str
        Nome do hiperparâmetro ('NSTEPS', 'EPISODIOS', 'ALPHA', 'GAMMA', 'EPSILON')
    valores : list
        Lista de valores a testar
    base_episodios, base_nstep, base_alpha, base_gamma, base_epsilon : float/int
    ...
    for valor in valores:
        ...
        # Prepara parâmetros do n-step SARSA
        kwargs = {
            'n': base_nstep,
            'gamma': base_gamma,
            'N': base_episodios,
        ...
        if nome_hiperparametro == 'NSTEPS':
            kwargs['n'] = valor
        elif nome_hiperparametro == 'EPISODIOS':
            kwargs['N'] = valor
        ...
        # Executa n-step SARSA
        env_exp = gym.make(ambiente, render_mode=render_mode)
        Q_exp, Pi_exp, _, _, T_exp, G_exp = n_step_sarsa(env_exp, **kwargs)
        env_exp.close()
...
# Valores a variar para cada hiperparâmetro
# NSTEPS: 5 valores distintos conforme a tarefa
VALORES_NSTEPS = [1, 2, 3, 5, 8]
VALORES_EPISODIOS = [3000, 5000, 7000]
VALORES_ALPHA = [0.001, 0.01, 0.1]
VALORES_GAMMA = [0.7, 0.9, 0.99]
VALORES_EPSILON = [0.05, 0.1, 0.2]

# %%
# Executa todos os experimentos
# Experimento 1: Variando NSTEPS (5 valores conforme a tarefa)
executar_experimento_hiperparametro(
    'NSTEPS', VALORES_NSTEPS,
    BASE_EPISODIOS, BASE_NSTEP, BASE_ALPHA, BASE_GAMMA, BASE_EPSILON, BASE_SEED
)

# %%
# Experimento 2: Variando EPISODIOS
executar_experimento_hiperparametro(
...
# %%
# Experimento 5: Variando EPSILON
executar_experimento_hiperparametro(
